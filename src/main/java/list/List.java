...
public abstract class List<A> {
    ...
    public <A1, A2> Tuple<List<A1>, List<A2>> unzip(Function<A, Tuple<A1, A2>> f) {
        return this.foldRight(new Tuple<>(list(), list()), a -> tl -> {
            ...
            return new Tuple<>(tl.fst.cons(t.fst), tl.snd.cons(t.snd));
        });
    }
    ...
    public Result<A> getAt(int index) {
        ...
        Tuple<Result<A>, Integer> rt = index < 0 || index >= length()
                ? identity
                : foldLeft(identity, ta -> a -> ta.snd < 0 ? ta : new Tuple<>(Result.success(a), ta.snd - 1));
        return rt.fst;
    }
    ...
    public boolean forAll(Function<A, Boolean> p) {
        ...
        return foldLeft(true, false, f).fst;
    }

    public boolean exists(Function<A, Boolean> p) {
        Function<Boolean, Function<A, Boolean>> f = x -> y -> x || p.apply(y);
        return foldLeft(false, true, f).fst;
    }
    ...
    public <B> List<Tuple<Result<A>, Result<B>>> zipAll(List<B> s2) {
        return zipWithAll(s2, tuple -> new Tuple<>(tuple.fst, tuple.snd));
    }

    public <B, C> List<C> zipWithAll(List<B> s2, Function<Tuple<Result<A>, Result<B>>, C> f) {
        Function<Tuple<List<A>, List<B>>, Result<Tuple<C, Tuple<List<A>, List<B>>>>> g = x -> x.fst.isEmpty() && x.snd.isEmpty()
                ? Result.empty()
                : x.snd.isEmpty()
                ? Result.success(new Tuple<>(f.apply(new Tuple<>(Result.success(x.fst.head()), Result.empty())), new Tuple<>(x.fst.tail(), List.<B> list())))
                : x.fst.isEmpty()
                ? Result.success(new Tuple<>(f.apply(new Tuple<>(Result.empty(), Result.success(x.snd.head()))), new Tuple<>(List.<A> list(), x.snd.tail())))
                : Result.success(new Tuple<>(f.apply(new Tuple<>(Result.success(x.fst.head()), Result.success(x.snd.head()))), new Tuple<>(x.fst.tail(), x.snd.tail())));
    ...
    private static class Cons<A> extends List<A> {
        ...
        private boolean isEquals(Cons<?> o) {
            ...
            return zipAll(o).foldRight(true, x -> y -> equals.apply(x.fst).apply(x.snd));
        }
        ...
        private List<Tuple<List<A>, List<A>>> split_(List<A> list) {
            ...
            return yss.map(t -> new Tuple<>(t.fst.cons(head), t.snd)).cons(new Tuple<>(list(head), tail));
        }
    }
    ...
    public static <A1, A2> Tuple<List<A1>, List<A2>> unzip(List<Tuple<A1, A2>> list) {
        return list.foldRight(new Tuple<>(list(), list()), t -> tl -> new Tuple<>(tl.fst.cons(t.fst), tl.snd.cons(t.snd)));
    }
    ...
    public static <A, S> List<A> unfold_(S z, Function<S, Result<Tuple<A, S>>> f) {
        return f.apply(z).map(x -> unfold_(x.snd, f).cons(x.fst)).getOrElse(list());
    }
    ...
    private static <A, S> TailCall<List<A>> unfold(List<A> acc, S z, Function<S, Result<Tuple<A, S>>> f) {
        ...
        Result<TailCall<List<A>>> result = r.map(rt -> sus(() -> unfold(acc.cons(rt.fst), rt.snd, f)));
